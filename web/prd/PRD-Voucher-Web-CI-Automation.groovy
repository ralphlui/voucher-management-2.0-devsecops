pipeline {
    ...
    environment {
        ...
        PROJECT_KEY = "ralphlui-voucher-app-web"
        ORGANIZATION_KEY = "ralphlui-voucher-management-web-frontend"
    ...
    stages {
        ...
        stage('Checkout & Install dependencies') {
            steps {
                checkout scmGit(branches: [[name: '*/develop']], extensions: [], userRemoteConfigs: [[credentialsId: 'a2b5c5aa-618f-44f3-80e3-2ee3c6775e46', url: 'git@github.com:ralphlui/voucher-app-web.git']])
        ...
        stage ('Build Docker image') {
            steps {
                script {
                    ...
                    echo "We will build latest docker image version: ${currentVersion}"

                    sh """
                    # Remove existing image if it exists
                    docker images -q ${IMAGE_NAME}-prd | xargs -r docker rmi -f || true
                    docker images -q ${IMAGE_NAME} | xargs -r docker rmi -f || true
                    docker build -t ${IMAGE_NAME}:v${currentVersion}-prd .
                    """
                }
            }
        }
        stage ('Push Docker image to AWS ECR') {
            steps {
                script {
                    ...
                    sh """
                    # Tag image
                    docker tag ${IMAGE_NAME}:v${currentVersion}-prd ${ECR_REPO_URI}/${IMAGE_NAME}:v${currentVersion}-prd
                    # Push to AWS ECR
                    docker push ${ECR_REPO_URI}/${IMAGE_NAME}:v${currentVersion}-prd
                    """
                }
            }
        }
