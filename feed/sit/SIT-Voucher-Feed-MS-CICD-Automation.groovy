pipeline {
    ...
    environment {
        SONAR_TOKEN = credentials('SONAR_TOKEN_VOUCHER_FEED')
        S3_BUCKET = "voucher-app-sit/feed"
    ...
    stages {
        stage('Run unit tests') {
            steps {
                checkout scmGit(branches: [[name: '*/development']], extensions: [], userRemoteConfigs: [[credentialsId: 'a2b5c5aa-618f-44f3-80e3-2ee3c6775e46', url: 'git@github.com:ralphlui/voucher-app-feed.git']])
                sh 'mvn -B clean test'
            }
        }
        ...
        stage ('Build Docker image') {
            steps {
                script {
                    ...
                    def filename = 'docker_version_feed.txt'
                    ...
                    if (fileExists(concatenatedPath)) {
                        ...
                        sh "cp ./docker_version_feed.txt ${VERSION_FILE_PATH}"
                    }
