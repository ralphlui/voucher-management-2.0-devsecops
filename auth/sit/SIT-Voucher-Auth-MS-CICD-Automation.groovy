pipeline {
    ...
    environment {
        SONAR_TOKEN = credentials('SONAR_TOKEN_VOUCHER_AUTH')
        S3_BUCKET = "voucher-app-sit/auth"
    ...
    stages {
        stage('Run unit tests') {
            steps {
                checkout scmGit(branches: [[name: '*/development']], extensions: [], userRemoteConfigs: [[credentialsId: 'a2b5c5aa-618f-44f3-80e3-2ee3c6775e46', url: 'git@github.com:ralphlui/voucher-app-auth.git']])
                sh 'mvn -B clean test'
            }
        }
            ...
            steps {
                script {
                    ...
                    dir("${BUILD_DIR}/workspace/${pipelineName}/target") {
                        sh """
                            ${SCA_SCAN} --project 'Voucher Management 2.0 (BackendAPI)' --scan .
        ...
        stage ('Build Docker image') {
            steps {
                script {
                    ...
                    def filename = 'docker_version_auth.txt'
                    ...
                    if (fileExists(concatenatedPath)) {
                        ...
                        sh "cp ./docker_version_auth.txt ${VERSION_FILE_PATH}"
                    }
